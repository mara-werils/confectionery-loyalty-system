...
export default function Swap() {
  ...
  const handleSwap = async () => {
    ...
    if (direction === 'SWEET_TO_TON' && amount > balances.SWEET) {
      toast.error('Insufficient SWEET balance');
      return;
    }
    ...
    toast.loading('Routing trade through DeDust...', { id: 'swap' });
    
    await new Promise(resolve => setTimeout(resolve, 3000));
    
    toast.success('Swap successful! Tokens transferred to your wallet.', { id: 'swap' });
    setIsSwapping(false);
    setPayAmount('');
    setReceiveAmount('');
  };
      ...
      >
        <div className="flex justify-between items-center mb-6 px-2">
            <div>
                ...
                <p className="text-sm text-zinc-500 mt-1">Trade SWEET instantly on TON DEX</p>
            </div>
          ...
          <div className="bg-black/40 rounded-3xl p-4">
            ...
            <div className="bg-[#0f0f11] border border-white/5 rounded-2xl p-4 mb-1 transition-colors hover:border-white/10 relative">
                <div className="flex justify-between mb-2">
                    ...
                    <span className="text-xs text-zinc-500 font-mono">
                        Balance: {balances[payToken].toLocaleString()}
                    </span>
                </div>
            ...
            <div className="bg-[#0f0f11] border border-white/5 rounded-2xl p-4 mt-1 transition-colors hover:border-white/10">
                <div className="flex justify-between mb-2">
                    ...
                    <span className="text-xs text-zinc-500 font-mono">
                        Balance: {balances[receiveToken].toLocaleString()}
                    </span>
                </div>
                ...
                {payAmount && Number(payAmount) > 0 && (
                    ...
                    >
                        <div className="flex justify-between items-center text-xs text-zinc-500 py-1">
                            <span className="flex items-center gap-1">Rate <InformationCircleIcon className="w-3 h-3" /></span>
            ...
            >
              {!wallet ? 'Connect Wallet' : isSwapping ? 'Swapping...' : 'Swap Tokens'}
            </button>
          </div>
        </GlassCard>
      </motion.div>
